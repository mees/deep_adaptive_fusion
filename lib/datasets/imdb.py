...
class imdb(object):
    ...
    def append_flipped_images(self):
        ...
        for i in xrange(num_images):
            ...
            #     print "oo0000000000"
            # for j in xrange(1,len(boxes)):
            #     boxes[j, 0] = widths[i] - oldx2[j] #- 1
            #     boxes[j, 2] = widths[i] - oldx1[j] #- 1
            #     if(boxes[j,2] < boxes[j,0]):
            #         print widths[i] - oldx2[j] #- 1
            #         print widths[i] - oldx1[j] #- 1
            #         print j , boxes[j,2], boxes[j,0], widths[i], oldx2[j], oldx1[j]
        ...
        for i in xrange(self.num_images):
            ...
            if gt_roidb is not None:
                if gt_roidb[i] is not None:
                    ...
                    total_num_fg_examples += np.count_nonzero( (overlaps[:,1] > cfg.TRAIN.FG_THRESH) ) + len(gt_boxes)
            ...
            # Test phase
                ...
                roidb.append({'boxes' : boxes,
                              ...
                              'gt_overlaps' : overlaps,
                              'flipped' : False})   
        print 'total_num_fg_examples with GT', total_num_fg_examples, ' for FG_TRESH ', cfg.TRAIN.FG_THRESH
        return roidb
