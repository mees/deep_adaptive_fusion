...
/**
 * @brief Gating layer for mixture of experts. assigns a weight for two input streams
 *
 * 
 */
template <typename Dtype>
class GatingInnerProductLayer : public Layer<Dtype> {
 public:
  explicit GatingInnerProductLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "GatingInnerProduct"; }
 ...
 protected:
  ...
  // Total Assignment of all experts
  vector<float> G_total;  
  float G_mean;  
  int M_;
  int K_;
  int N_;
